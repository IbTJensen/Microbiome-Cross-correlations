...
cors <- list()
set.seed(1671522881)
# Omits Ccamk since no correlations were found
for(i in c(1,3:5)){
  cat(i, "\r")
...
gg <- ggarrange(g[[1]], g[[4]], g[[5]], g[[3]],
                common.legend = T,
                legend = "bottom",
                labels = c(gt[c(1,4,5,3)]),
                label.x = 0.7)

ggsave("../Supplementary figures/SFig8.pdf", gg, width = 200, height = 160, units = "mm")

# Saving correlation results as table
Cors_filt <- rbindlist(cors)
fwrite(Cors_filt, "../Tables/S3 Table.csv")
