...
# source("../Functions/SparXCC_function.R")
...
set.seed(1713034557)
CC_XCC <- SparXCC_base(OTU.abn = t(Bac1),
                       gene.expr = t(Fung1),
                       pseudo_count = 1,
                       Find_m = T,
                       cores = 1,
                       B_m = 100)
colnames(CC_XCC$cor) <- Fung$OTUid
rownames(CC_XCC$cor) <- Bac$OTUid
...
sum(CC_spear$p<0.001 & CC_XCC$Correlated)/sum(CC_spear$p<0.001)
# Proportion of TSS-pearson correlations also found found by SparXCC
sum(CC_pears$p<0.001 & CC_XCC$Correlated)/sum(CC_XCC$Correlated)
...
long_CLR <- melt(CC_XCC$cor, value.name = "SparXCC")
long_CLR_p <- melt(CC_XCC$Correlated, value.name = "SparXCC")
...
ggsave("../Supplementary figures/SFig9.pdf", g, width = 200, height = 120, units = "mm")
...
fwrite(long_CLR, "../Tables/S4 Table.csv")

# Examining the correlations where Spearman and SparXCC disagree on the sign
w <- which(sign(CC_XCC$cor) != sign(CC_spear$r) & CC_XCC$Correlated, arr.ind = TRUE)
data.table(Bac_OTU = rownames(CC_XCC$cor)[w[,1]],
           Fung_OTU = colnames(CC_XCC$cor)[w[,2]]) -> D
...
w <- which(sign(CC_XCC$cor) != sign(CC_pears$r) & CC_XCC$Correlated, arr.ind = TRUE)
data.table(Bac_OTU = rownames(CC_XCC$cor)[w[,1]],
           Fung_OTU = colnames(CC_XCC$cor)[w[,2]]) -> D2
