...
for(i in 1:5){
  ...
  C <- SparXCC(t(Bac_log), t(Fung_log), pseudo_count = 0)
  cor_thresh <- sapply(1:20,
                       function(x) SparXCC(t(Bac_log)[sample(1:N[i]),],
                                           t(Fung_log)[sample(1:N[i]),],
                                           pseudo_count = 0) %>% abs() %>% quantile(1 - 1/(p[i]*q[i])) ) %>% mean()
