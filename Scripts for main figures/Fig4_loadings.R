...
OTU_thresh_mat <- matrix(OTU_thresh_vec, 3, 3)
rownames(OTU_thresh_mat) <- as.character(c(10,100,1000))
colnames(OTU_thresh_mat) <- as.character(c(10,100,1000))

Gene_thresh_mat <- matrix(Gene_thresh_vec, 3, 3)
rownames(Gene_thresh_mat) <- as.character(c(10,100,1000))
...
R <- foreach(
  ...
  mae_XCC2 <- data.table(Opt[i,],
                         MAE = sapply(seq(0,0.8,0.1), function(x) mae(true.cor[abs(true.cor)>x], XCC2[abs(true.cor)>x])),
                         Threshold = seq(0,0.8,0.1),
                         Method = "SparXCC iterative")
  B <- rbind(mae_C, mae_CC, mae_CLR, mae_mix, mae_XCC, mae_XCC2)
}
Sys.time() - a
...
ggplot(data = R3_yes_main, aes(x = Threshold, y = MAE, ymin=MAE_min, ymax=MAE_max, col = Method, fill = Method, group = Method))+
  geom_line(linewidth = 1, aes(linetype = "Non-zero"))+
  geom_line(linewidth = 1, aes(linetype = "Zero"))+
...
save(file = "../Temp/g2_yes.txt", g2_yes_main)
save(file = "../Temp/g2_no.txt", g2_no_main)

ggsave(filename = "../Supplementary figures/SFig5.pdf", g2_supp, width = 190, height = 160, unit = "mm")
