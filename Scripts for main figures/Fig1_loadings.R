...
R <- foreach(
  ...
  if(!all(is.na(CEV2$cor))){
    mae_CEV2 <- data.table(Opt[i,],
                           MAE = sapply(seq(0,0.8,0.1), function(x) mae(true.cor[abs(true.cor)>x], CEV2$cor[abs(true.cor)>x])),
                           Threshold = seq(0,0.8,0.1),
                           Method = "SparCEV iterative")
  }
  
  B <- rbind(mae_C, mae_CC, mae_CLR, mae_CEV, mae_CEV2); B
}
Sys.time() - a
...
save(file = "../Temp/g2_yes.txt", g2_yes)
save(file = "../Temp/g2_no.txt", g2_no)
